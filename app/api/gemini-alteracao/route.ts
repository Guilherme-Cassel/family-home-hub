 ...
 */
function sanitizar(bruto: unknown, catalogo: ItemCatalogo[]): AlteracaoIA[] {
  ...
  for (const linha of bruto) {
    ...
    const item =
      Number.isInteger(indice) && indice >= 0 && indice < catalogo.length
        ? catalogo[indice]
        : null
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ alteracoes: sanitizar(bruto, itens) })
  } catch (erro) {
